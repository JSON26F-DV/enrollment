...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($role) || !in_array($role, ['admin', 'staff', 'student']))
        ...
        if (empty($errors)) {
            if ($is_edit) {
                if (!empty($password)) {
                    ...
                    $stmt = $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, email = ?, birthday = ?, gender = ?, contact_number = ?, role = ?, status = ?, password = ? WHERE id = ?");
                    $stmt->execute([$first_name, $last_name, $email, $birthday, $gender, $contact_number, $role, $status, $hashed, $edit_id]);
                } else {
                    $stmt = $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, email = ?, birthday = ?, gender = ?, contact_number = ?, role = ?, status = ? WHERE id = ?");
                    $stmt->execute([$first_name, $last_name, $email, $birthday, $gender, $contact_number, $role, $status, $edit_id]);
                }
                $success = 'Account updated successfully!';
            } else {
                $hashed = password_hash($password, PASSWORD_DEFAULT);
                $stmt = $pdo->prepare("INSERT INTO users (first_name, last_name, birthday, gender, contact_number, email, password, role, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$first_name, $last_name, $birthday, $gender, $contact_number, $email, $hashed, $role, $status, $_SESSION['user_id']]);
...
$page_title = $is_edit ? 'Edit Account' : 'Register Account';
$current_role = $old['role'] ?? ($is_edit ? $user['role'] : 'student');
$current_status = $old['status'] ?? ($is_edit ? $user['status'] : 'pending');
?>
...
<div class="max-w-[1040px] mx-auto px-4 md:px-8 py-12">
    ...
    <?php else: ?>
        ...
        <div class="flex justify-center">
            <div class="border border-black/10 rounded-[32px] p-6 md:p-8 bg-white w-full max-w-xl">
                <form method="POST" action="" novalidate>
                    ...
                    <div class="space-y-4">
                        <div class="flex gap-4">
                            <div
                                class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5 flex-1">
                        ...
                        <div class="flex justify-between gap-2">
                            <div
                                class="relative border border-black/10 rounded-xl flex-1 px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                            ...
                            <div
                                class="relative flex-1 border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                <label class="text-xs font-medium text-black/50">Gender</label>
                                <select name="gender" required
                                    class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1">
                                    <option value="" disabled <?= empty($old['gender'] ?? ($is_edit ? $user['gender'] : '')) ? 'selected' : '' ?>>Select</option>
                            ...
                            <div
                                class="relative border flex-1 border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                <label class="text-xs font-medium text-black/50">Role</label>
                                <select name="role" required
                                    ...
                                    <option value="student" <?= $current_role === 'student' ? 'selected' : '' ?>>Student
                                    </option>
                                </select>
                            </div>
                            <div
                                ...
                                <select name="status" required
                                    class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1">
                                    <option value="pending" <?= $current_status === 'pending' ? 'selected' : '' ?>>Pending
                                    </option>
                                    <option value="active" <?= $current_status === 'active' ? 'selected' : '' ?>>Active
                                    </option>
                                    <option value="inactive" <?= $current_status === 'inactive' ? 'selected' : '' ?>>Inactive
                                    </option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-3 mt-6">
                        <button type="submit" name="save"
                            class="px-6 py-2.5 text-sm font-bold text-white bg-google-blue hover:bg-google-blue-hover rounded-full transition-colors shadow-sm active:scale-95"><?= $is_edit ? 'Update Account' : 'Register Account' ?></button>
                        <a href="<?= url('/src/view/admin/landing/dashboard.php') ?>"
