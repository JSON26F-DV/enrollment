        ...
        if (empty($errors)) {
            if ($is_edit) {
                if (!empty($password)) {
                    ...
                    $stmt = $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, email = ?, birthday = ?, gender = ?, contact_number = ?, password = ? WHERE id = ?");
                    $stmt->execute([$first_name, $last_name, $email, $birthday, $gender, $contact_number, $hashed, $edit_id]);
                } else {
                    $stmt = $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, email = ?, birthday = ?, gender = ?, contact_number = ? WHERE id = ?");
                    $stmt->execute([$first_name, $last_name, $email, $birthday, $gender, $contact_number, $edit_id]);
                }
                $success = 'Student updated successfully!';
            } else {
                $hashed = password_hash($password, PASSWORD_DEFAULT);
                $stmt = $pdo->prepare("INSERT INTO users (first_name, last_name, birthday, gender, contact_number, email, password, role, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, 'student', 'pending', ?)");
                $stmt->execute([$first_name, $last_name, $birthday, $gender, $contact_number, $email, $hashed, $_SESSION['user_id']]);
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div
                            ...
                            <input type="text" value="<?= $is_edit ? htmlspecialchars($user['status']) : 'Pending' ?>"
                                disabled
                                class="w-full bg-transparent text-sm font-medium text-black/50 outline-none mt-1 py-1">
                            <input type="hidden" name="status"
                                value="<?= $is_edit ? htmlspecialchars($user['status']) : 'pending' ?>">
                        </div>
                    </div>
