    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            try {
                ...
                $document_types = [
                    'psa_birth_certificate', 'form_138', 'good_moral', 
                    'certificate_of_graduation', 'id_photo_2x2', 'valid_id',
                    'tor', 'honorable_dismissal'
                ];
                    ...
                    if (isset($_FILES[$doc_type]) && $_FILES[$doc_type]['error'] === UPLOAD_ERR_OK) {
                        ...
                        if (in_array($file['type'], $allowed_types) && $file['size'] <= 5 * 1024 * 1024) {
                            ...
                            if (move_uploaded_file($file['tmp_name'], $dest_path)) {
                                ...
                                ");
                                $stmt_doc->execute([
                                    $applicant_id, $doc_type, $file['name'], $relative_path, 
                                    $file['size'], $file['type']
                                ]);
                            }
                        }
...
<head>
    ...
    <style>
        ...
        .education-section { display: none; }
        .education-section.active { display: block; }
    </style>
</head>
...
<body>
    ...
    <nav class="absolute top-0 left-0 w-full p-4 md:p-8 z-10">
        <a href="<?= url('/src/view/guest/landing/landingpage.php') ?>" class="inline-flex items-center gap-2 text-sm font-bold text-black/60 hover:text-black transition-colors">
        ...
        <div class="w-full max-w-[1040px] bg-white rounded-[32px] md:rounded-[40px] shadow-2xl overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 p-6 md:p-8 text-white">
                <div class="flex items-center gap-4">
                    <img src="<?= url('/public/images/ncst.png') ?>" alt="NCST" class="w-12 h-12 rounded-xl object-cover bg-white">
                    <div>
            ...
            <div class="p-6 md:p-8">
                ...
                <?php if ($success): ?>
                    <div class="text-center py-8">
                        ...
                        <a href="<?= url('/src/view/auth/login/loginpage.php') ?>" class="px-6 py-2.5 text-sm font-bold text-white bg-google-blue hover:bg-google-blue-hover rounded-full transition-colors shadow-sm inline-block">
                            Go to Login
                        </a>
                    </div>
                <?php else: ?>
                    ...
                    <form id="applicationForm" method="POST" action="" novalidate enctype="multipart/form-data">
                        ...
                        <div class="mb-6">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-xs font-medium text-black/50">Step <span id="currentStepNum">1</span> of <?= count($step_labels) ?></span>
                                <span id="stepLabelText" class="text-xs font-medium text-google-blue"><?= $step_labels[0] ?></span>
                            </div>
                            <div class="flex gap-1.5">
                                <?php for ($i = 1; $i <= count($step_labels); $i++): ?>
                                    <div class="h-1.5 flex-1 rounded-full step-dot <?= $i === 1 ? 'bg-google-blue' : 'bg-black/20' ?>" data-step="<?= $i ?>"></div>
                                <?php endfor; ?>
                            </div>
                        </div>
                        ...
                        <div class="step-content active" data-step="1">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="suffix" value="<?= htmlspecialchars($old['suffix'] ?? '') ?>"
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="date" name="birthday" id="birthday" value="<?= htmlspecialchars($old['birthday'] ?? '') ?>" required
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="nationality" value="<?= htmlspecialchars($old['nationality'] ?? 'Filipino') ?>"
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="religion" value="<?= htmlspecialchars($old['religion'] ?? '') ?>"
                                ...
                                <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="birth_place" value="<?= htmlspecialchars($old['birth_place'] ?? '') ?>"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="email" name="email" value="<?= htmlspecialchars($old['email'] ?? '') ?>" required
                                        class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1 placeholder:text-black/30" placeholder="juan.delacruz@email.com">
                                </div>
                                <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    <label class="text-xs font-medium text-black/50">Mobile Number *</label>
                                    <input type="tel" name="contact_number" value="<?= htmlspecialchars($old['contact_number'] ?? '') ?>" required
                                ...
                                <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="home_address" value="<?= htmlspecialchars($old['home_address'] ?? '') ?>"
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="province" value="<?= htmlspecialchars($old['province'] ?? '') ?>"
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="zip_code" value="<?= htmlspecialchars($old['zip_code'] ?? '') ?>"
                        ...
                        <div class="step-content" data-step="3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="text" name="guardian_name" value="<?= htmlspecialchars($old['guardian_name'] ?? '') ?>"
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                    ...
                                    <input type="tel" name="guardian_contact" value="<?= htmlspecialchars($old['guardian_contact'] ?? '') ?>"
                                        class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1 placeholder:text-black/30" placeholder="09123456789">
                                </div>
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 bg-black/5">
                        ...
                        <div class="step-content" data-step="4">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex flex-wrap gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="education_type" value="freshman" <?= ($old['education_type'] ?? 'freshman') === 'freshman' ? 'checked' : '' ?> onchange="toggleEducationFields()">
                                        <span class="text-sm">Freshman</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="education_type" value="transferee" <?= ($old['education_type'] ?? '') === 'transferee' ? 'checked' : '' ?> onchange="toggleEducationFields()">
                            ...
                            <!-- Freshman Fields -->
                            <div id="freshmanFields" class="education-section <?= ($old['education_type'] ?? 'freshman') === 'freshman' ? 'active' : '' ?>">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                        <label class="text-xs font-medium text-black/50">Senior High School Name</label>
                                        <input type="text" name="highschool_name" value="<?= htmlspecialchars($old['highschool_name'] ?? '') ?>"
                                    ...
                                    <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                        ...
                                        <input type="text" name="highschool_address" value="<?= htmlspecialchars($old['highschool_address'] ?? '') ?>"
                                    ...
                                    <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                        ...
                                        <input type="text" name="year_graduated" value="<?= htmlspecialchars($old['year_graduated'] ?? '') ?>"
                                    ...
                                    <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                        ...
                                        <input type="text" name="lrn" value="<?= htmlspecialchars($old['lrn'] ?? '') ?>" maxlength="12"
                            ...
                            <!-- Transferee Fields -->
                            <div id="transfereeFields" class="education-section <?= ($old['education_type'] ?? '') === 'transferee' ? 'active' : '' ?>">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                        <label class="text-xs font-medium text-black/50">Previous College/University</label>
                                        <input type="text" name="previous_college" value="<?= htmlspecialchars($old['previous_college'] ?? '') ?>"
                                    ...
                                    <div class="relative border border-black/10 rounded-xl px-4 py-2 focus-within:border-google-blue focus-within:ring-1 focus-within:ring-google-blue transition-all bg-black/5">
                                        ...
                                        <input type="text" name="previous_course" value="<?= htmlspecialchars($old['previous_course'] ?? '') ?>"
                        ...
                        <div class="step-content" data-step="5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 bg-black/5">
                                    ...
                                    <select name="preferred_course" required class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1">
                                        <option value="">Select Course</option>
                                        <?php foreach ($courses as $course): ?>
                                            <option value="<?= htmlspecialchars($course['code']) ?>" <?= ($old['preferred_course'] ?? '') === $course['code'] ? 'selected' : '' ?>>
                                ...
                                <div class="md:col-span-2 relative border border-black/10 rounded-xl px-4 py-2 bg-black/5">
                                    ...
                                    <select name="second_course" class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1">
                                        <option value="">Select Course</option>
                                        <?php foreach ($courses as $course): ?>
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 bg-black/5">
                                    ...
                                    <select name="academic_year_id" required class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1">
                                ...
                                <div class="relative border border-black/10 rounded-xl px-4 py-2 bg-black/5">
                                    ...
                                    <select name="semester" required class="w-full bg-transparent text-sm font-medium text-black outline-none mt-1 py-1">
                            ...
                            <div class="mt-6 p-4 bg-blue-50 rounded-xl border border-blue-200">
                                ...
                                <div class="grid grid-cols-2 gap-2 text-sm">
                                    <div><span class="text-blue-600">Course:</span> <span class="font-medium">BS Information Technology</span></div>
                                    <div><span class="text-blue-600">Academic Year:</span> <span class="font-medium">2026-2027</span></div>
                                    <div><span class="text-blue-600">Semester:</span> <span class="font-medium">1st Semester</span></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="step-content" data-step="6">
                            <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                                <span class="w-6 h-6 rounded-full bg-google-blue text-white text-xs flex items-center justify-center">6</span>
                                Upload Requirements
                            </h3>
                            <p class="text-sm text-gray-600 mb-4">Upload clear images/scans of the following documents. Files should be JPG, PNG, or PDF (max 5MB each).</p>
                            
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <!-- PSA Birth Certificate -->
                                <div>
                                    <label class="text-xs font-medium text-black/50 mb-1 block">PSA Birth Certificate *</label>
                                    <div class="file-upload-area" onclick="document.getElementById('psa_birth_certificate').click()">
                                        <input type="file" id="psa_birth_certificate" name="psa_birth_certificate" accept=".jpg,.jpeg,.png,.pdf" class="hidden" onchange="updateFileName(this)">
                                ...
                                <!-- Form 138 -->
                                <div>
                                    <label class="text-xs font-medium text-black/50 mb-1 block">Form 138 / Report Card</label>
                                    <div class="file-upload-area" onclick="document.getElementById('form_138').click()">
                                        <input type="file" id="form_138" name="form_138" accept=".jpg,.jpeg,.png,.pdf" class="hidden" onchange="updateFileName(this)">
                                        <svg class="w-8 h-8 mx-auto text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                        </svg>
                                        <p class="text-sm text-gray-500">Click to upload</p>
                                ...
                                <!-- Good Moral -->
                                <div>
                                    <label class="text-xs font-medium text-black/50 mb-1 block">Good Moral Certificate</label>
                                    <div class="file-upload-area" onclick="document.getElementById('good_moral').click()">
                                        <input type="file" id="good_moral" name="good_moral" accept=".jpg,.jpeg,.png,.pdf" class="hidden" onchange="updateFileName(this)">
                                        <svg class="w-8 h-8 mx-auto text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                        </svg>
                                        <p class="text-sm text-gray-500">Click to upload</p>
                                ...
                                <div>
                                    <label class="text-xs font-medium text-black/50 mb-1 block">Certificate of Graduation (optional)</label>
                                    <div class="file-upload-area" onclick="document.getElementById('certificate_of_graduation').click()">
                                        <input type="file" id="certificate_of_graduation" name="certificate_of_graduation" accept=".jpg,.jpeg,.png,.pdf" class="hidden" onchange="updateFileName(this)">
                                        <svg class="w-8 h-8 mx-auto text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                        </svg>
                                ...
                                <div>
                                    ...
                                    <div class="file-upload-area" onclick="document.getElementById('id_photo_2x2').click()">
                                        <input type="file" id="id_photo_2x2" name="id_photo_2x2" accept=".jpg,.jpeg,.png" class="hidden" onchange="updateFileName(this)">
                                        <svg class="w-8 h-8 mx-auto text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                        </svg>
                                        <p class="text-sm text-gray-500">Click to upload</p>
                                ...
                                <div>
                                    ...
                                    <div class="file-upload-area" onclick="document.getElementById('valid_id').click()">
                                        <input type="file" id="valid_id" name="valid_id" accept=".jpg,.jpeg,.png,.pdf" class="hidden" onchange="updateFileName(this)">
                                        <svg class="w-8 h-8 mx-auto text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                        </svg>
                                        <p class="text-sm text-gray-500">Click to upload (optional)</p>
                                ...
                                <!-- TOR (for transferees) -->
                                <div>
                                    <label class="text-xs font-medium text-black/50 mb-1 block">Transcript of Records (Transferees only)</label>
                                    <div class="file-upload-area" onclick="document.getElementById('tor').click()">
                                        <input type="file" id="tor" name="tor" accept=".jpg,.jpeg,.png,.pdf" class="hidden" onchange="updateFileName(this)">
                                        <svg class="w-8 h-8 mx-auto text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                        </svg>
                                ...
                                <!-- Honorable Dismissal -->
                                <div>
                                    <label class="text-xs font-medium text-black/50 mb-1 block">Honorable Dismissal (Transferees only)</label>
                                    <div class="file-upload-area" onclick="document.getElementById('honorable_dismissal').click()">
                                        <input type="file" id="honorable_dismissal" name="honorable_dismissal" accept=".jpg,.jpeg,.png,.pdf" class="hidden" onchange="updateFileName(this)">
                                        <svg class="w-8 h-8 mx-auto text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                        </svg>
                                        <p class="text-sm text-gray-500">Click to upload (for transferees)</p>
                            ...
                            <div class="mt-6 p-4 bg-amber-50 rounded-xl border border-amber-200">
                                <p class="text-sm text-amber-800">
                                    <strong>Note:</strong> Documents are optional at this stage. You can upload them later if needed. However, your application will be processed faster if all documents are submitted.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-8 pt-6 border-t border-black/10">
                            <button type="button" id="backBtn" onclick="prevStep()" class="px-6 py-2 text-sm font-bold text-google-blue hover:bg-google-blue/10 rounded-full transition-colors active:scale-95">
                                ← Back
                            </button>
                            <div class="flex gap-2">
                                <button type="button" id="nextBtn" onclick="nextStep()" class="px-6 py-2.5 text-sm font-bold text-white bg-google-blue hover:bg-google-blue-hover rounded-full transition-colors shadow-sm active:scale-95">
                                    Next →
                                </button>
                                <button type="submit" name="submit_application" id="submitBtn" class="px-6 py-2.5 text-sm font-bold text-white bg-green-600 hover:bg-green-700 rounded-full transition-colors shadow-sm active:scale-95 hidden">
                                    Submit Application ✓
                                </button>
                            </div>
                        </div>
